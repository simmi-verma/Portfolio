...
const SkillCategoryCard = ({ category, items, index, catIcon }) => {
  return (
    <motion.div
      ...
      className="w-full h-[450px] bg-white/60 backdrop-blur-md rounded-2xl border border-slate-200 p-6 shadow-sm flex flex-col items-center justify-center hover:shadow-md hover:border-[#BA90C6]/50 transition-all duration-300 relative overflow-hidden group/card"
    >
      {/* Subtle background glow effect behind image */}
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[120px] h-[120px] bg-[#E8A0BF]/20 blur-[40px] pointer-events-none rounded-full"></div>

      <div className="mb-8 flex flex-col items-center justify-center w-full gap-5 relative z-10 text-center">
        {catIcon && (
          <div className="w-[80px] h-[80px] rounded-2xl p-[2px] bg-gradient-to-br from-[#E8A0BF] to-[#BA90C6] shadow-[0_0_20px_rgba(186,144,198,0.2)] group-hover/card:shadow-[0_5px_15px_rgba(232,160,191,0.3)] transition-shadow duration-500">
            <img
              src={catIcon}
              alt={category}
      ...
      <div className="flex flex-wrap justify-center gap-3">
        {items.map((item, itemIdx) => (
          <div
            key={itemIdx}
            className="flex items-center gap-2 bg-white/80 hover:bg-white py-2 px-3.5 rounded-lg border border-slate-200 shadow-sm hover:shadow-md hover:border-[#BA90C6]/50 transition-all duration-300 cursor-default group"
          >
            <img
              src={item.icon}
              alt={item.name}
            ...
            />
            <span className="text-slate-600 group-hover:text-[#BA90C6] drop-shadow-sm text-[14px] font-semibold tracking-wide transition-colors">
              {item.name}
            </span>
          </div>
        ))}
